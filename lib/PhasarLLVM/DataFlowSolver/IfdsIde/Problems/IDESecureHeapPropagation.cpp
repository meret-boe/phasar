...
bool IDESecureHeapPropagation::IdentityEdgeFunction::equalTo(
...
bool IDESecureHeapPropagation::SHPGenEdgeFn::equalTo(
    std::shared_ptr<EdgeFunction<l_t>> Other) const {
...
IDESecureHeapPropagation::SHPGenEdgeFn::getInstance(l_t Val) {
  ...
  if (!Cache[Ind]) { //NOLINT
    Cache[Ind] = std::make_shared<SHPGenEdgeFn>(Val); //NOLINT
  }
  return Cache[Ind]; //NOLINT
}
} // namespace psr
