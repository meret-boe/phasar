...
public:
  ...
  virtual bool equalTo(const mono_container_t &Lhs,
                        const mono_container_t &Rhs) = 0;
  ...
  [[nodiscard]] std::set<std::string> getEntryPoints() const { return EntryPoints; }

  [[nodiscard]] const ProjectIRDB *getProjectIRDB() const { return IRDB; }

  [[nodiscard]] const TypeHierarchy<t_t, f_t> *getTypeHierarchy() const { return TH; }

  [[nodiscard]] const c_t *getCFG() const { return CF; }

  [[nodiscard]] const PointsToInfo<v_t, n_t> *getPointstoInfo() const { return PT; }
